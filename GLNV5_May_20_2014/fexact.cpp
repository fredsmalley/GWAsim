// fexact.cpp -- Standalone C++ fisher exact test in C++.
//   Modfied from fexact.c used in R-project, downloaded from
//   http://svn.r-project.org/R/trunk/src/library/stats/src/fexact.c
//
// Joe Song
// June 2, 2013
...
#ifdef USING_R      // MS 6/2/2013
#include <R.h>
#else                   // MS 6/2/2013
#include <cmath>        // MS 6/2/2013
#include <cstdlib>      // MS 6/2/2013
#include <cstdio>       // MS 6/2/2013
#include <algorithm>    // MS 6/2/2013
#include <iostream>     // MS 6/2/2013
#include <vector>

using namespace std;    // MS 6/2/2013

typedef bool Rboolean;  // MS 6/2/2013
#define R_alloc calloc  // MS 6/2/2013
#define REprintf printf // MS 6/2/2013

#define FALSE false     // MS 6/2/2013
#define TRUE true       // MS 6/2/2013

#define fmax2 max       // MS 6/2/2013
#define fmin2 min       // MS 6/2/2013

#define imax2 max       // MS 6/2/2013
#define imin2 min       // MS 6/2/2013

#define new newvar      // MS 6/2/2013
...
LoopNode: /* Generate a node */
    ...
    if (nro == 2) { /* Only 1 row left */
        ...
        if (key < -1) {
            // PROBLEM "Bug in FEXACT: gave negative key" RECOVER(NULL_ENTRY); MS 6/2/2013
            cerr << "Bug in FEXACT: gave negative key" << endl; // MS 6/2/2013
            exit(EXIT_FAILURE); // MS 6/2/2013
        }
        /* Table index */
        ipn = key % ldst + 1;
        /* Find empty position */
...
void prterr(int icode, const char *mes)
{
    /*
     ...
     -----------------------------------------------------------------------
     */
    // PROBLEM "FEXACT error %d.\n%s", icode, mes RECOVER(NULL_ENTRY); MS 6/2/2013
    
    cerr << "FEXACT error " << icode << '.' << endl << mes << endl; // MS 6/2/2013
    exit(EXIT_FAILURE); // MS 6/2/2013
...
double gammds(double *y, double *p, int *ifault)
{
    ...
    *ifault = 0;
    ...
    g = 1.;
    a = *p;
//L10: MS. 4/15/2014: Commented out L10 to suppress warning during compilation
    do {
        a += 1.;
        c *= (*y / a);
        g += c;
...
/* MS 6/2/2013 Commented out the following:
...
double Fexact(const vector< vector<int> > & X)
{
    ...
    int *x = (int *) calloc(nr * nc, sizeof(int));
    ...
    fexact(&nr, &nc, x, &nr, &expect, &percnt, &emin, &ptable, &pval, &ws, &mult);

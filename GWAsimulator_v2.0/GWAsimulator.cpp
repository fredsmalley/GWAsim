                            ...
                            )
{
   ...
   for(i=0; i<NUMCASEF; i++){
      ...
      //printf("index to disease intex = %d, dd = %d, NUMDL = %d\n", index, dd, NUMDL);
      for(j=0; j<NUMDL; j++){
         if(dd[j]==0) {sim_hap[j][2*i][mkpos[j]] = false; sim_hap[j][2*i+1][mkpos[j]] = false;}
         else if(dd[j]==1) {sim_hap[j][2*i][mkpos[j]] = false; sim_hap[j][2*i+1][mkpos[j]] = true;}
         else {sim_hap[j][2*i][mkpos[j]] = true; sim_hap[j][2*i+1][mkpos[j]] = true;}
         //("Female disease location %d, is at mkpos %d, with allele 1 = %d, allele 2 = %d\n", j+1, mkpos[j], allele1, allele2);
         mkPOS[j] = mkpos[j];
         //sim_hap[j][2*i][mkpos[j]] = allele1;
         //sim_hap[j][2*i+1][mkpos[j]] = allele2;
         
         printf("allele 1 = %d, allele 2 = %d\n", sim_hap[j][2*i][mkpos[j]] ? 1 : 0, sim_hap[j][2*i+1][mkpos[j]] ? 1 : 0);
      }
      //printf("\n");
   }
   ...
   for(i=NUMCASEF; i<step2; i++){
      ...
      for(j=0; j<NUMDL; j++){
         ...
         //printf("Male disease location %d, is at mkpos %d, with allele 1 = %d, allele 2 = %d\n", j+1, mkpos[j], allele1, allele2);
         mkPOS[j] = mkpos[j];
         //sim_hap[j][2*i][mkpos[j]] = allele1;
         //sim_hap[j][2*i+1][mkpos[j]] = allele2;
         printf("allele 1 = %d, allele 2 = %d\n", sim_hap[j][2*i][mkpos[j]] ? 1 : 0, sim_hap[j][2*i+1][mkpos[j]] ? 1 : 0);
      ...
      if(NUMDL==0 | REGIONAL ==0 | i<NUMDL) {
         ...
         if(!strcmp(OUTFORMAT, "linkage")) {
            ...
            for(j=0; j<n_person; j++) {
               ...
               if ( (sim_hap[i][2*j][mkPOS[i]] ? 1 : 0 ) && (sim_hap[i][2*j+1][mkPOS[i]] ? 1 : 0 )){
                  if ((rand() % 100) > 30 )
                     status = 1;
                  
                  //if ((rand() % 100) > 75 )
                  //   status = 1;
               } else if ( (sim_hap[i][2*j][mkPOS[i]] ? 0 : 1 ) && (sim_hap[i][2*j+1][mkPOS[i]] ? 0 : 1 )){
                  //odds = 100;//(rand() % 100) * .5;
               } else {
                  if ((rand() % 100) > 50 )
                     status = 1;
               ...
               for(k=startpos; k<=endpos; k++){
                  count++;
                  //outfile << (sim_hap[i][2*j][k] ? 1 : 0 ) << " " << (sim_hap[i][2*j+1][k] ? 1 : 0) << " ";
                  
                   if ((sim_hap[i][2*j][k] ? 1 : 0 ) && (sim_hap[i][2*j+1][k] ? 1 : 0)){
                   outfile << "2 ";
                   } else if ((sim_hap[i][2*j][k] ? 0 : 1) && (sim_hap[i][2*j+1][k] ? 0 : 1)){
                   outfile << "0 ";
                   } else {
                   outfile << "1 ";
                   }

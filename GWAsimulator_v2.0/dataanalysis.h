...
{
   cout << "in data analysis" << endl;
   
   map<int, string, greater<int> > chisqMap;
   int i, j, k, countMarker, NUMMARKERTOTAL;
   ...
   for (i = 0; i < 7; ++i){
      for (j = 0; j < numMarker[i]; ++j, ++k){
         sprintf(chr, "chr%d_%d", Chr[i], j);
         chisqMap.emplace(chisq[k]*1000, chr);
      }
   }
   
   k = 0;
   for(auto const &it1 : chisqMap) {
      cout << "Loci = " << it1.second << " chisq value = " << it1.first << endl;
      if (k++ >= 100)
         break;
